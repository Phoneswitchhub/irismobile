// Automatically generated Thailand Address Data
...
// Helper to initialize Thailand address cascading dropdowns
function initThailandAddressSelects(provinceSelectId, districtSelectId, initialProvince = '', initialDistrict = '') {
  ...
  THAILAND_PROVINCES.forEach(p => {
    ...
    opt.value = p.name_en;
    opt.textContent = `${p.name_en} (${p.name_th})`;
  ...
  function updateDistrictOptions(provName, distName) {
    ...
    const provinceData = THAILAND_PROVINCES.find(p => p.name_en === provName);
    if (provinceData && provinceData.districts) {
      provinceData.districts.forEach(d => {
        const opt = document.createElement('option');
        opt.value = d.name_en;
        opt.textContent = `${d.name_en} (${d.name_th})`;
